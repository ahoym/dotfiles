...
## `getOrderbook()` Doesn't Support All Parameters

The xrpl.js sugar method `client.getOrderbook()` does NOT support newer protocol parameters (e.g., `domain` for permissioned DEX order books). When you need unsupported params, fall back to raw `client.request({ command: "book_offers", ... })`. With raw requests, you need two requests (one per direction) and must normalize the offer format yourself.
