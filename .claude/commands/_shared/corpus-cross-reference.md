...
Shared procedure for loading the target corpus and assessing content coverage. Used by curate-learnings, curate-sync, and any skill that needs to evaluate content against existing knowledge.
...
**Note:** `<TARGET>` is the repo root — for curate-learnings this is typically `~/.claude/`, for curate-sync it's the current project root.
