...
Skill instructions should reference artifacts generically, not by the tool that created them. Other engineers may use different tools to produce the same outputs.

- ❌ "check if the project has a CLAUDE.md generated by `/explore-repo`"
- ✅ "check if the project has a CLAUDE.md"

This extends the "Genericize Project-Specific Content" principle (see `learnings/guideline-authoring.md`) to tool references within skill instructions themselves.
