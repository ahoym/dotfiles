...
| 3 | GUIDELINES | Wire unreferenced guideline | `guidelines/validation.md` | CLAUDE.md `@`-reference | MEDIUM | blocked | `validation.md` (12 lines, 2 patterns) is not `@`-referenced in CLAUDE.md and not referenced by any skill — never loaded. CLAUDE.md is outside `.claude/` write scope. Whether to add always-on cost is preference-dependent. See blockers.md [B-1]. |
