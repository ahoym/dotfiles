...
- For resolved and acknowledged comments, post the emoji reaction only — no text reply. The reaction is the acknowledgement; a text reply just re-iterates the addresser's fix and adds noise. Text replies are reserved for partially-addressed and not-addressed cases that carry new content. See `review-comment-classification.md`.
