...
- Verify all financial arithmetic uses `BigNumber.js` — never use `parseFloat()` or native operators (`+`, `-`, `*`, `/`) on prices, amounts, totals, or spreads (see `~/.claude/learnings/financial/numeric-precision-strategy.md` for patterns)
...
- `~/.claude/learnings/financial/numeric-precision-strategy.md` — BigNumber.js rules for financial arithmetic, comparison traps, precision strategy
...
- `~/.claude/learnings/financial/domain-ledger-architecture.md` — core ledger schema, balance composition, reconciliation (via fintech-ledger-engineer persona for crypto ledger work)
