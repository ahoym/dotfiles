...
- **Compound auto-relaunch**: after every runner completion, run this decision tree automatically — do not prompt the operator unless escalation is needed. Offset cadence follows `director-playbook.md`'s Loop Setup — event-driven is the standard; the 3-min offset is the cron-scheduled legacy fallback.
