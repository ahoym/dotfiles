...
**GitLab:** `glab api projects/:id/merge_requests/{PR_NUMBER} | jq -r '(.head_pipeline.status // .pipeline.status)'` until terminal (`success`, `failed`, `canceled`, `skipped`). (Coalesce the two with `//` — GitLab populates `.pipeline` vs `.head_pipeline` inconsistently across versions, so take whichever is non-null; when both are `null` right after a push the pipeline isn't created yet — keep polling rather than treating null as terminal.) For job-level detail: `glab api "projects/:id/pipelines/<pipeline-id>/jobs" | jq -r '.[] | {name, status}'`. Failing logs: `glab api projects/:id/jobs/<job-id>/trace`. (GitLab status values are lowercase; map `success`→pass, `failed`→fail.)
