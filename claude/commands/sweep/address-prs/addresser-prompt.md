...
**GitLab:** `glab api projects/:id/merge_requests/{PR_NUMBER} | jq -r '.pipeline.status'` until terminal (`success`, `failed`, `canceled`, `skipped`). (Use `.pipeline`, not the undocumented/inconsistently-populated `.head_pipeline`; a `null` right after a push means the pipeline isn't created yet — keep polling rather than treating null as terminal.) For job-level detail: `glab api "projects/:id/pipelines/<pipeline-id>/jobs" | jq -r '.[] | {name, status}'`. Failing logs: `glab api projects/:id/jobs/<job-id>/trace`. (GitLab status values are lowercase; map `success`→pass, `failed`→fail.)
