...
- `MODEL` → `"claude-opus-4-7"` (leaf — reads diffs, edits files, pushes commits; standard 200k context is the safe default. Opt into `[1m]` only on credit-enabled accounts with large diffs — see sweep-scaffold.md MODEL selection)
