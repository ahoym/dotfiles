...
Patterns for maintaining consistency when ledger entries span multiple services, databases, or legal entities.
...
- `~/.claude/learnings/domain-ledger-architecture.md` — entry lifecycle, balance composition, intercompany account structure, and double-entry constraint (saga patterns build on these foundations)
...
- `~/.claude/learnings/financial-applications.md` — two-layer idempotency and fail-loudly error handling (non-obvious complement to saga idempotency key strategy)
