...
Patterns for applying event sourcing and CQRS to financial ledger systems — event store design, CQRS separation, snapshots, projections, schema evolution, and how ES layers onto the three-table schema.
...
- `~/.claude/learnings/domain-ledger-architecture.md` — three-table schema, balance composition, entry lifecycle (the base layer ES enhances)
...
- `~/.claude/learnings/financial-applications.md` — monetary calculation safety and idempotency patterns for event handlers
...
- `~/.claude/learnings/spring-boot.md` — Spring transactional event listener patterns, Spring Data JPA with immutable entities
