...
1. **Tilde vs absolute paths:** Permission patterns like `Write(~/**/tmp/claude-artifacts/**)` require the tool call to use `~/...` paths. Passing the expanded absolute path (`/Users/<user>/...`) won't match the tilde-based pattern.
