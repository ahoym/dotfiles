...
- **Keywords:** trust arc, agent-to-agent review, verify assumptions, gate announcements, intent files, TaskOutput, subagent verification, front-load context, persona routing, architecture reviewer, defensive coding, adapter code, research PR review, cited-number provenance, findings doc, positive-signal vs finding, orthogonal-aspect merge, dissent over-trigger, fan-out adjudication, per-cluster verifier, addresser-side verification, identity footnote ownership
- **Related:** none
...
## Adjudicating a Large Review Batch: Fan Out Per-Cluster Verifiers, Then Own the Footnote

When *addressing* a big review (20+ comments), group the comments by file-cluster and spawn one subagent per cluster that **independently verifies each reviewer claim against the PR-head files** (`git show <head_sha>:<path>` after `git fetch origin <pr_branch>` — the working tree sits on the base branch) *before* drafting a reply. Independent verification routinely catches reviewer errors a rubber-stamp would miss: a cross-ref checked in the wrong directory, a "genericization leak" that's actually the real repo owner, a "missing" section that exists at a different path. Each agent returns a verdict (agree / partial / disagree) + a grounded draft; the orchestrator **appends the identity footnote uniformly** rather than trusting each agent to format it — the `Role:` footer is the self-filter / mutual-resolution key, so own it centrally. Sibling to the reviewer-side "verify the cited file before posting" (above), applied on the addresser side.
